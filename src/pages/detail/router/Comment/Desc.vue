<template lang="html">
  <ul class="comment_content">
    <li v-for="(value,index) of desc" :key="index">
      <p>{{((commentCount-1)-index==0?"沙发":((commentCount-1)-index==1?"板凳":((commentCount-1)-index==2?"地板":commentCount-index+'楼')))}}</p>
      <div class="main">
        <div class="top_img">
          <div class="img"></div>
          <p>{{value.username}}</p>
...
export default {
  mounted(){
    ...
    this.$store.commit('getBigComment',{mid:this.mid,type:'single',order:'desc',cache:false});
    $(window).off('scroll');
    $(window).on('scroll',(e)=>{
      ...
      if((scrollTop+clientHeight)>=scrollHeight){
        setTimeout(()=>{
          this.$store.commit('getBigComment',{mid:this.mid,type:'single',order:'desc',cache:true});
        },100);
      }
    });
  },
  ...
  computed:{
    ...mapGetters(['checkLogin','desc','smallComments','commentCount']),
  },
  methods:{
    //回复
    reply(id){
