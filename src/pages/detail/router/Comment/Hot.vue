<template lang="html">
  <ul class="comment_content">
    <li v-for="(value,index) of hot" :key="index">
...
export default {
  mounted(){
    ...
    this.$store.commit('getBigComment',{mid:this.mid,type:'single',order:'hot',cache:false});
    $(window).off('scroll');
    $(window).on('scroll',(e)=>{
      ...
      if((scrollTop+clientHeight)>=scrollHeight){
        setTimeout(()=>{
          this.$store.commit('getBigComment',{mid:this.mid,type:'single',order:'hot',cache:true});
        },100);
      }
    });
  },
  ...
  computed:{
    ...mapGetters(['checkLogin','hot','smallComments','commentCount']),
  },
  methods:{
    //回复
    reply(id){
