...
export default {
  ...
  methods:{
    getSearchData(){   //获取搜索的数据
      $.ajax({
        async: true,
        url: "https://c.y.qq.com/soso/fcgi-bin/client_search_cp?ct=24&qqmusic_ver=1298&remoteplace=txt.yqq.lyric&searchid=100897311650111851&aggr=0&catZhida=1&lossless=0&sem=1&t=7&g_tk=5381&loginUin=0&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0",
        type: "GET",
        dataType: 'jsonp',
        jsonp: 'jsonpCallback',
        data: {
          ...
          p:this.page,
          n:this.num,
          'jsonpCallback':'MusicJsonCallback19603019257876575'
        },
        timeout: 5000,
        success: (json) => {
          var song = json.data.lyric;
          ...
          console.log(list);
            ...
            this.searchData.push({
              'img':'https://y.gtimg.cn/music/photo_new/T002R300x300M000'+value.albummid+'.jpg?max_age=2592000',
              'single':value.songname,
              'subtitle':'',
              'singer':value.singer[0].name,
              'singerMid':value.singer[0].mid,
              'singleMid':value.songmid,
              'meidaMid':value.media_mid,
            })
          })
          // console.log(list);
